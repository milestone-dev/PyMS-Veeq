...
aicontrol_names = {
	...
	0x5: 'no_retaliation',
	0x6: 'focus_disabled',
	0x7: 'dont_focus_disabled',
	0x8: 'unit_mover_enabled',
	0x9: 'unit_mover_disabled',
}
...
class AIBIN:
	...
	command_aliases = { # those will expand for non script commands, not compatible with script!
		...
		"bld_need": "player_need",
		"upg": "upgrade",
		"wait_upg": "wait_upgrades",
		"w_upg": "wait_upgrades",
		"w_bld": "wait_build",
		"w_bldstart": "wait_buildstart",
		"wait_bld": "wait_build",
		"wait_bldstart": "wait_buildstart",
		"w": "wait",
		"check_townpoint": "try_townpoint",
		"expand_townpoint": "allies_watch",
		"transfer_workers": "get_oldpeons",
		"maynard": "get_oldpeons",
		"bring": "bring_jump",
		"local": "local_jump",
		"resarea": "resarea_jump",
		"notowns": "notowns_jump",
		"upg_j": "upgrade_jump",
		"tech_j": "tech_jump",
		"rand_j": "random_jump ",
	...
	short_labels = [
		...
		'defense', 			  #0x9d
		'max_build',		  #0x9e
		'attack_to_deaths',   #0x9f,
		'container_jump',	  #0xa0,
		'script',	          #0xa1,
		'local_jump', 		  #0xa2,
		'resarea_jump',		  #0xa3,
		'set_free_id',        #0xa4,
		'set_pos_variable',   #0xa5
		'set_var_local',   	  #0xa6
	]
	...
	def __init__(self, bwscript=None, units=None, upgrades=None, techs=None, stat_txt=None):
		...
		self.parameters = [
			...
			#upgrade_jump
			[self.ai_byte, self.ai_compare_trig, self.ai_word, self.ai_byte, self.ai_address],
			#tech_jump
			...
			#max_build
			[self.ai_byte,self.ai_unit,self.ai_word],
			#attack_to_deaths
			[self.ai_unit,self.ai_word, self.ai_unit,self.ai_word, self.ai_unit,self.ai_word, self.ai_unit,self.ai_word],
			#container_jump
			[self.ai_resource_type,self.ai_compare_trig,self.ai_dword,self.ai_area,self.ai_address],
			#script
			[self.ai_string],
			#local_jump
			[self.ai_byte,self.ai_compare_trig,self.ai_dword,self.ai_unit_or_group,self.ai_address],
			#resarea_jump
			[self.ai_byte,self.ai_compare_trig,self.ai_byte,self.ai_resource_type,self.ai_dword,self.ai_address],
			#set_free_id
			None,
			#set_pos_variable
			[self.ai_address, self.ai_string],
			# set_var_local
			[self.ai_address, self.ai_string, self.ai_byte],
		]
		self.builds = []
				...
				if loc:
					...
					while curoffset < len(curdata):
						...
						if not cmd and curoffset == len(curdata):
							break
						if cmd >= len(self.short_labels):
							raise PyMSError('Load','Invalid command, could possibly be a corrupt aiscript.bin (command: %s)' % cmd)
						ai = [cmd]
	...
	def interpret(self, files, defs=None, extra=False):
		...
		def parse_param(parse_fn,d,n=None,line=None):
			try:
				...
				def try_substitute(name):
					...
					if lower in variables:
						for pt in self.typescanbe[parse_fn.__doc__.split(' ',1)[0]]:
							...
							var_error = PyMSError(
								'Variable',
								"Incorrect type on variable '%s'. Expected '%s' but got '%s'" % (
									lower,
				...
				if var_error:
					raise var_error 
				e.line = -1 #n + 1 # trying to evaluate 'n' caused crash
				e.code = line
				e.warnings = warnings
				if var:
							...
							if ai:
								...
								if match:
									...
									if not re.match(r"[ \t]*script.*", line):
										...
										# if script command then combine arguments to one
										combinedArgs = ",".join(dat)
										dat = [combinedArgs]
									aisize += 1
									if params and dat:
			...
			for i in findgoto.iteritems():
				...
				for l,f in i[1].iteritems():
					if not f[0]:
						...
						add_warning(PyMSWarning('Interpeting',"The label '%s' in AI script '%s' is unused." % (l,i[0])))
				...
				if loc:
					...
					while curoffset < len(curdata):
						...
						if not cmd and curoffset == len(curdata):
							break
						if cmd > len(self.short_labels):
							raise PyMSError('Load','Invalid command, could possibly be a corrupt bwscript.bin (command: %s)' % cmd)
						ai = [cmd]
						if self.parameters[cmd]:
							for p in self.parameters[cmd]:
