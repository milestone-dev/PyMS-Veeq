...
class Tileset:
	...
	def load_file(self, cv5, vf4=None, vx4=None, vr4=None, dddata=None, wpe=None):
		...
		if isstr(cv5):
			...
			cv5expanded = False
			if name.split(os.extsep)[-1].lower() == 'cv5ex':
				name = name[:-6]
				cv5expanded = True
	...
	def save_file(self, cv5, vf4=None, vx4=None, vr4=None, dddata=None, wpe=None):
		path = os.path.dirname(cv5)
		name = os.path.basename(cv5)
		if name.endswith(os.extsep + 'cv5ex'):
			name = name[:-6]
		if name.endswith(os.extsep + 'cv5'):
			name = name[:-4]
		cv5expanded = 'ex' if self.cv5.expanded else ''
		cv5 = os.path.join(path, '%s%scv5%s' % (name,os.extsep,cv5expanded))
		if vf4 == None:
			vf4 = os.path.join(path, '%s%svf4' % (name,os.extsep))
		if vx4 == None:
...
class CV5:
	...
	def load_file(self, file, expanded=None):
		...
		if expanded == None and isstr(file):
			expanded = (file[-6:].lower() == '.cv5ex')
		data = load_file(file, 'CV5')
		struct_size = (84 if expanded else 52)
		file_type = 'Expanded CV5 file' if expanded else 'CV5 file'
		if data and len(data) % struct_size:
			raise PyMSError('Load',"'%s' is an invalid %s" % (file, file_type))
		groups = []
		try:
			struct_frmt = '<HBB24L' if expanded else '<HBB24H'
			o = 0
			while o + 51 < len(data):
				d = list(struct.unpack(struct_frmt, data[o:o+52]))
	...
	def save_file(self, file):
		struct_frmt = '<HBB24L' if self.expanded else '<HBB24H'
		data = ''
		for d in self.groups:
			data += struct.pack(struct_frmt, *[d[0],(d[1] << 4) + d[2],(d[3] << 4) + d[4]] + d[5:13] + d[13])
		if isstr(file):
			try:
