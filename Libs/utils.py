...
class InternalErrorDialog(PyMSDialog):
	...
	def widgetize(self):
		...
		Label(self, text='The PyMS program "%s" has encountered an unknown internal error.\nThe program will attempt to continue, but may cause problems or crash once you press Ok.\nYou could try to contact poiuy_qwert, the original creator, but you he is recently inactive in the community.\nYou can however contact Veeq7, the author of PyMS patch. You should send send this traceback with any relivant information' % self.prog, justify=LEFT).pack(side=TOP, padx=2, pady=2, fill=X)
		r = Frame(self)
		Hotlink(r, 'Contact poiuy_qwert', self.contact).pack(side=RIGHT, padx=10, pady=2)
		Hotlink(r, 'Contact Veeq7', self.contactVeeq).pack(side=RIGHT, padx=30, pady=2)
		r.pack(fill=X)
	...
	def contactVeeq(self, e=None):
		askquestion(title='Veeq7 Contact Information', message='Discord: Veeq7#0592\nYou can also find me on Pr0nogo\'s discord server.', type=OK)
