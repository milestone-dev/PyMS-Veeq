...
attack_add max(unit_count(Military) - Byte(1), Byte(2)) Military"""),
    ...
    ('Flow control',[
        ...
        ('if_attacking','Jumps to Block if the AI is currently preparing an attack.'),
    ]),
    ('Multiple threads',[
...
It seems that AI difficulty is mostly an unused concept. The AI Difficulty now is always 1, unless the AI has never started a town, in which case it is 0. There is also some unused functionality which would allow AI to mine more than 8 resources per trip if the difficulty was ever 2."""),
        ...
        ('max_workers','Sets maximum workers for the current town. 255 to restore default logic'),
    ]),
    ('StarEdit',[
...
class CodeEditDialog(PyMSDialog):
    ...
    def widgetize(self):
        buttons = [
            ...
            10,
            ('asc3topyai', self.asc3topyai, 'Compile ASC3 to PyAI (Ctrl+W)', '<Control-w>'),
    ...
    def expandScriptAlias(self, command):
        ...
        newcmd = re.sub(r"(^[ \t]*)\b"+self.makeSearchPattern(AIBIN.AIBIN.script_aliases.keys())+r"\b(.*)",
                        lambda matchobj, dict=AIBIN.AIBIN.script_aliases: self.getReplacement(matchobj, dict, 2, matchobj.group(1), matchobj.group(3)), newcmd)
        self.scriptAliasesTime += self.stopBenchmark(benchmark)
        return newcmd
    ...
    def convertParanthesis(self, line): # used to convert (X Y) -> (X,Y) for script calls
        return re.sub(r"\(([0-9]+) ([0-9]+)\)", r"(\1,\2)", line)

    def getTimeAliasRegex(self, alias):
        return r"^([ \t]*)" + re.escape(alias) + r" ([0-9.]+)(.*)"
    ...
    def expandTimeAlias(self, line): # used to convert time aliases (aka waitmin 1 -> wait 1440)
        ...
        timeLiteral = re.sub(r'(\..*)\.', r'\1', timeLiteral) # remove repeating dots
    ...
    def expandArgumentAliases(self, line):
        ...
        searchPattern = r"\b"+self.makeSearchPattern(AIBIN.AIBIN.argument_aliases.keys())+r"\b"
        line = re.sub(searchPattern, lambda matchobj, dict=AIBIN.AIBIN.argument_aliases: self.getReplacement(matchobj, dict), line)

        self.argumentAliasesTime += self.stopBenchmark(benchmark)
        return line

    def startBenchmark(self):
        if not hasattr(self, "benchmark_id"):
            self.benchmark_id = 0
            self.benchmark = []
        else:
            self.benchmark_id += 1
        self.benchmark.append(0.0)
        self.benchmark[self.benchmark_id] = time.time()
        return self.benchmark_id

    def stopBenchmark(self, id):
        return time.time() - self.benchmark[id]

    def findAllBlocks(self, text):
        return re.findall(r"^[ \t]*:([a-zA-Z0-9_\-]+).*$", text, re.MULTILINE)

    def makeSearchPattern(self, array):
        if len(array) == 0:
            return None
        pattern = r"("
        for var in array:
    ...
    def getReplacement(self, matchobj, dict, groupid=1, prefix="", suffix=""):
        match = matchobj.group(groupid)
        ret = prefix + dict[match] + suffix
        return ret
    ...
    def asc3topyai(self, e=None):
        ...
        for line in text.split('\n'):
            ...
            if line.lstrip().startswith(';'):
                ...
                scriptTuple = self.convertScriptLine(line, blockSearch)
                for var in scriptTuple[1]:
                    vars.add(var)
                data += self.convertParanthesis(scriptTuple[0])
                data += " " + comment
                data += "\n"
            elif line.strip():
                line = self.expandArgumentAliases(line)
                ...
                if d[0] in AIBIN.AIBIN.short_labels or d[0] in AIBIN.AIBIN.command_aliases:
                    ...
                    self.commandAliasesTime += self.stopBenchmark(benchmark)
        ...
        beforeheader += "# Script Var Time: " + str(self.scriptVarTime) + "\n"

        afterheader = "\n"
        for var in vars:
            afterheader += "\tset_var_local(" + var + ", " + var + ", 13)\n"

        self.text.delete('1.0', END)
        self.text.insert(END, beforeheader + '\n' + header % tuple(headerinfo) + afterheader + data)
    ...
    def load(self):
        try:
            ...
            ErrorDialog(self, e)
            return
        if warnings and not self.parent.suppressInterpreter:
...
class ExternalDefDialog(PyMSDialog):
    ...
    def add(self, key=None):
        iimport = self.parent.select_file('Add External Definition File', True, '.txt', [('Text Files','*.txt *.asc3'),('All Files','*')], self)
...
class PyAI(Tk):
    def __init__(self, guifile=None):
        ...
        #Window
        Tk.__init__(self)
        self.minsize(400, 350) # TODO: Make this less hardcode-ish
        ...
        #PyAI Settings
        self.suppressInterpreter = self.settings.get("suppressInterpreter", False)
        self.suppressOverwrite = self.settings.get("suppressOverwrite", False)
        self.suppressOpenSave = self.settings.get("suppressOpenSave", False)
        ...
        menus['Edit'] = [
            ...
            ('Manage PyAI Settigns', self.managesettings, NORMAL, 'Ctrl+K', 7), # 29
        ]
        menus['View'] = [
        ...
        bars = [
            ...
            [
                ...
                ('asc3topyai', self.managesettings, 'Manage PyAI Settings (Ctrl+K)', NORMAL),
                4,
    ...
    def resort(self):
        ...
        if hasattr(self, 'storedscroll'):
            self.restoreScroll()

    def select_aiscript(self, title, open=True, parent=None):
        return self.select_file(title, open, '.bin', [('AI Script Bin','aiscript.bin'),('AI Scripts','*.bin'),('All Files','*')], parent, initialfile="aiscript.bin")

    def select_bwscript(self, title, open=True, parent=None):
        return self.select_file(title, open, '.bin', [('BW Script Bin','bwscript.bin'),('AI Scripts','*.bin'),('All Files','*')], parent, initialfile="bwscript.bin")

    def select_file(self, title, open=True, ext='.bin', filetypes=[('AI Scripts','*.bin'),('All Files','*')], parent=None, initialfile=""):
        if parent == None:
            parent = self
        path = self.settings.get('lastpath', BASE_DIR)
        parent._pyms__window_blocking = True
        file = [tkFileDialog.asksaveasfilename,tkFileDialog.askopenfilename][open](parent=parent, title=title, defaultextension=ext, filetypes=filetypes, initialdir=path, initialfile=initialfile)
    ...
    def open(self, key=None, aiscript=None, bwscript=None):
        if not self.unsaved():
            if not aiscript:
                aiscript = self.select_aiscript('Open aiscript.bin')
                if not aiscript:
                    return
                if not bwscript:
                    bwscript = self.select_bwscript('Open bwscript.bin (Cancel to only open aiscript.bin)')
            warnings = []
            try:
            ...
            if warnings and not self.parent.suppressOpenSave:
                WarningDialog(self, warnings)
    ...
    def saveas(self, key=None):
        if key and self.buttons['saveas']['state'] != NORMAL:
            return
        aiscript = self.select_aiscript('Save aiscript.bin As', False)
        if not aiscript:
            return True
        bwscript = None
        if self.ai.bwscript.ais:
            bwscript = self.select_bwscript('Save bwscript.bin As (Cancel to save aiscript.bin only)', False)
    ...
    def storeScroll(self):
        self.storedscroll = self.listbox.yview()[0]

    def restoreScroll(self):
        self.listbox.yview("moveto", self.storedscroll)
    ...
    def iimport(self, key=None, iimport=None, c=True, parent=None):
        ...
        if iimport:
            ...
            try:
                ...
                return -1
            cont = c
            if warnings and not self.suppressOpenSave:
                w = WarningDialog(parent, warnings, True)
                cont = w.cont
            if cont:
                for id,ai in i.ais.iteritems():
                    if (id in self.ai.ais and (cont == True or cont != 2)) and not self.suppressOverwrite:
    ...
    def managesettings(self, *_):
        PyAISettingsDialog(self, (340,295))
...
class PyAISettingsDialog(PyMSDialog):
    def __init__(self, parent, min_size):
        self.parent = parent
        self.min_size = min_size
        self.settings = parent.settings
        PyMSDialog.__init__(self, parent, "PyAI Settings")
    ...
    def widgetize(self):
        ...
        self.suppressInterpreter = BooleanVar()
        self.suppressInterpreter.set(self.parent.suppressInterpreter)
        Checkbutton(settings, text="Suppress Interpreter Warnings", variable=self.suppressInterpreter).pack(anchor="w", pady=3)

        self.suppressOverwrite = BooleanVar()
        self.suppressOverwrite.set(self.parent.suppressOverwrite)
        Checkbutton(settings, text="Suppress Script Overwrite Warnings", variable=self.suppressOverwrite).pack(anchor="w", pady=3)

        self.suppressOpenSave = BooleanVar()
        self.suppressOpenSave.set(self.parent.suppressOpenSave)
        Checkbutton(settings, text="Suppress Open/Save Warnings", variable=self.suppressOpenSave).pack(anchor="w", pady=3)

        tabs.add_tab(settings, "Suppress Warnings")
        tabs.pack(fill=BOTH, expand=1, padx=5, pady=5)

        btns = Frame(self)
        ok = Button(btns, text='Ok', width=10, command=self.ok)
        ok.pack(side=LEFT, padx=3, pady=3)
        Button(btns, text='Cancel', width=10, command=self.cancel).pack(padx=3, pady=3)
    ...
    def setup_complete(self):
        ...
        elif 'pyaisettingswindow' in self.settings:
            loadsize(self, self.settings, 'pyaisettingswindow', True)

    def cancel(self):
        PyMSDialog.ok(self)

    def save_settings(self):
        self.parent.suppressInterpreter = self.suppressInterpreter.get()
        self.parent.suppressOverwrite = self.suppressOverwrite.get()
        self.parent.suppressOpenSave = self.suppressOpenSave.get()

        self.settings["suppressInterpreter"] = self.suppressInterpreter.get()
        self.settings["suppressOverwrite"] = self.suppressOverwrite.get()
        self.settings["suppressOpenSave"] = self.suppressOpenSave.get()
    ...
    def dismiss(self, *_):
        if isinstance(self.settings, SettingDict):
            ...
            savesize(self, self.settings, 'pyaisettingswindow', True)
