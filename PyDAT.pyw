	...
	def play(self):
		if play_sound:
			try:
				...
				askquestion(title="Error", message="Can't play this sound.", type=OK)
...
class TechnologyTab(DATTab):
	...
	def __init__(self, parent, toplevel):
		...
		makeCheckbox(f, self.unused, 'Researched', 'TechUnused', self).pack(side=LEFT)
		makeCheckbox(f, self.broodwar, 'BroodWar', 'TechBW', self).pack(side=LEFT)
...
class UpgradesTab(DATTab):
	...
	def __init__(self, parent, toplevel):
		...
		makeCheckbox(s, self.broodwar, 'BroodWar', 'UpgIsBW', self).pack()
		s.pack(fill=BOTH, padx=5, pady=5)
		l.pack(side=LEFT)
		m.pack(fill=X)
		frame.pack(side=LEFT)
...
class AIActionsUnitsTab(DATUnitsTab):
	def __init__(self, parent, toplevel, parent_tab):
		...
		f = Frame(s)
		# AI Flags
		makeCheckbox(f, self.AI_NoSuicide, 'Ignore Strategic Suicide missions', 'UnitAINoSuicide', self).pack(side=LEFT)
		makeCheckbox(f, self.AI_NoGuard, 'Don\'t become a guard', 'UnitAINoGuard', self).pack(side=LEFT)
	...
	def __init__(self, parent, toplevel, parent_tab):
		...
		for lt,lf in flags:
			...
			for c in lf:
				...
				for t,v,h in c:
					f = Frame(cc)
					makeCheckbox(f, v, t, h, self).pack(side=LEFT)
					f.pack(fill=X)
				cc.pack(side=LEFT, fill=Y)
			s.pack(fill=BOTH, padx=5, pady=5)
...
class AdvancedUnitsTab(DATUnitsTab):
	def __init__(self, parent, toplevel, parent_tab):
		...
		for c in u:
			...
			for t,v in c:
				f = Frame(cc)
				makeCheckbox(f,v,'0x'+t,'UnitMov'+t, self).pack(side=LEFT)
				f.pack(fill=X)
			cc.pack(side=LEFT)
...
class PyDAT(Tk):
	def __init__(self, guifile=None):
		...
		listmenu = [
			('Copy Entry (Ctrl+C)', lambda t=0: self.copy(t), 0, '<Control-c>'), # 0
			('Paste Entry (Ctrl+V)', lambda t=0: self.paste(t), 0, '<Control-v>'), # 1
			None,
			('Copy Tab (Ctrl+Shift+C)', lambda t=1: self.copy(t), 1, '<Control-Shift-c>'), # 3
			('Paste Tab (Ctrl+Shift+V)', lambda t=1: self.paste(t), 1, '<Control-Shift-v>'), # 4
			None,
