	...
	def __init__(self, parent, ibin, ecallback=None, icallback=None, scallback=None, highlights=None):
		...
		if highlights:
			...
			self.highlights = {
				...
				'HeaderStart':{'foreground':'#2244FF','background':None,'font':self.boldfont},
				#'Types':{'foreground':'#2244FF','background':None,'font':self.boldfont},
				'Animations':{'foreground':'#2244AA','background':None,'font':self.boldfont},
				'Commands':{'foreground':'#2244AA','background':None,'font':None},
				'Number':{'foreground':'#FF02222','background':None,'font':None},
...
class PyICE(Tk):
	def __init__(self, guifile=None):
		...
		for n,l in enumerate(['IScript Entries','Images','Sprites',"Flingy's",'Units']):
			...
			scrollbar = Scrollbar(listframe)
			lb = Listbox(listframe, activestyle=DOTBOX, selectmode=EXTENDED, font=couriernew, width=1, height=1, bd=0, highlightthickness=0, yscrollcommand=scrollbar.set, exportselection=0)

			lb.bind('<Button-1>', lambda e, l=lb: self.deselectOthers(e, l))
			lb.bind('<Control-Button-1>', self.blank)
			if n == 0:
				self.iscriptlist = lb
			elif n == 1:
				self.imageslist = lb
	...
	def deselectOthers(self, e, currentList):
		for list in self.getAllScrollLists():
			if list != currentList:
	...
	def getAllScrollLists(self):
		yield self.iscriptlist
		yield self.imageslist
		yield self.spriteslist
		yield self.flingylist
		yield self.unitlist

	def storeScroll(self):
		self.storedscroll = []
		for i,list in enumerate(self.getAllScrollLists()):
			x = i
			y = list
			self.storedscroll.append(list.yview()[0])

	def restoreScroll(self):
		for i,list in enumerate(self.getAllScrollLists()):
			list.yview("moveto", self.storedscroll[i])
