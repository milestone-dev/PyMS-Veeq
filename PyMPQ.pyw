...
class FolderDialog(PyMSDialog):
	...
	def ok(self, *_):
		PYMPQ_SETTINGS['import'].add_folder = self.result.get().replace("/", "\\")
		self.save = True
		PyMSDialog.ok(self)
...
class PyMPQ(Tk):
	def __init__(self, guifile=None):
		...
		self.listbox = ReportList(self, ['Name','Size','Ratio','Packed','Locale','Attributes',None], EXTENDED, self.select, self.do_rename, self.popup, self.openfile, min_widths=[50]*6, background=stylized_theme_default_background)
	...
	def adddir(self, key=None):
		...
		if fo.save:
			...
			for p in os.walk(path):
				path_folder = p[0][len(path):] + "\\"
				folder = PYMPQ_SETTINGS['import'].get('add_folder', '') + path_folder.replace("/", "\\")
				for f in p[2]:
					if fc == None or PYMPQ_SETTINGS.get('compress')[0] == 4:
						fc = self.flagcomp(f)
					try:
						MpqAddFileToArchiveEx(h, os.path.join(path, path_folder, f), folder + f, *fc)
					except:
						ErrorDialog(self, PyMSError('Write MPQ (Update Files)', "The MPQ could not be opened. Other non-PyMS programs may lock MPQ's while open. Please try closing any programs that might be locking your MPQ."))
						MpqCloseUpdatedArchive(h)
						return
	...
	def extract(self, key=None):
		try:
			...
			askquestion(title="Error", message="Can't extract this file.", type=OK)
