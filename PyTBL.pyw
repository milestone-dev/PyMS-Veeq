...
class PyTBL(Tk):
	...
	def open(self, key=None, file=None):
		if not self.unsaved():
			if not file:
				file = self.select_file('Open TBL or Extended TBL', filetypes=[('StarCraft TBL Files','*.tbl *.tblex'),('All Files','*')])
				if not file:
					return
			tbl = TBL.TBL()
			try:
	...
	def save(self, key=None):
		...
		try:
			...
			if warning is not None:
				askquestion(parent=self, title="You have reached the limit", message=warning, type=OK)
		except PyMSError, e:
			ErrorDialog(self, e)
	...
	def saveas(self, key=None):
		if key and self.buttons['saveas']['state'] != NORMAL:
			return
		file = self.select_file('Save TBL As', False, ext='.tbl', filetypes=[('StarCraft TBL Files','*.tbl'),('Extended StarCraft TBL Files','*.tblex'),('All Files','*')])
		if not file:
			return True
		self.file = file
		self.save()
	...
	def register(self, e=None):
		try:
			...
			register_registry('PyTBL','','tblex',os.path.join(BASE_DIR, 'PyTBL.pyw'),os.path.join(BASE_DIR,'Images','PyTBL.ico'))
		except PyMSError, e:
			ErrorDialog(self, e)
		...
		if opt.gui:
			...
			try:
				if opt.convert:
					...
					warning = tbl.compile(args[1])
					if warning is None:
						print " - '%s' written succesfully" % args[1]
					else:
						print " - '%s' %s" % (args[1], warning)
			except PyMSError, e:
				print repr(e)
